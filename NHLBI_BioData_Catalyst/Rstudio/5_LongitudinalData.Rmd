...
# Create summary table by pivoting the dataframe to the number of exams that are associated with each variable
longitudinal_lipid_summary <- filtered_lipid_df %>% select(varname_noexam, exam_number) %>% pivot_wider(names_from = varname_noexam, values_from = exam_number, values_fn = length) %>% select_if(longitudinal_lipid_summary > 1)
```
