...
PICSURE_network_URL = "https://predev.picsure.biodatacatalyst.nhlbi.nih.gov/picsure"
...
```{r}
raw_df <- raw_df %>% dplyr::filter(`\\phs002299\\bl_sex\\` != '')
```
...
simplified_names <- names(raw_df) %>%
  stringr::str_extract('([\\w\\s\\(\\)]+(?=\\\\$))|(Patient\\sID)')
...
orchid_data$vs_died <- forcats::fct_recode(orchid_data$vs_died, '0' = 'No', '1' = 'Yes') %>%
  as.character() %>%
  as.numeric()
...
```{r}
demographics_table <- arsenal::tableby(rand_trt ~ 
                             bl_sex +
                             bl_age +
                             race_ethnicity 
                             , data = orchid_table1)

baseline_table <- arsenal::tableby(rand_trt ~ 
                             bl_randloc +
                             covid_ooscale_1 +
                             vit_vasop_1 
                             , data = orchid_table1)

comorbidity_table <- arsenal::tableby(rand_trt ~ 
                             diabetes_bool +
                             kidney_bool +
                             cad_bool +
                             hypertension_bool +
...
symptoms_table <- arsenal::tableby(rand_trt ~
                          ie_arisymptoms___cou +
                          ie_arisymptoms___fev +
                          ie_arisymptoms___sob +
                          ie_arisymptoms___st,  
...
admission_table <- arsenal::tableby(rand_trt ~ d_sofa_gcs + d_onset_duration,
                              data = orchid_table1
)
...
dplyr::filter(!Characteristic %in% c('FALSE', 'Range'))
```
...
html_out1 <- kableExtra::kbl(table1_df, caption = "Table 1: Description of the population") %>%
kableExtra::kable_minimal() %>% kableExtra::kable_styling(bootstrap_options = c("striped"), 
                                  full_width = FALSE,
                                  position = "left", 
                                 font_size = 12) %>%
...
kableExtra::add_header_above(c(" ", "No. (%)" = 2), align = 'left') %>%
kableExtra::row_spec(0, bold = TRUE, extra_css = "font-size: medium") %>%
kableExtra::row_spec(indices_cat, underline = FALSE, bold = TRUE, align = 'left') %>%
kableExtra::column_spec(1, background = 'FDEBD0') %>%
kableExtra:::add_indent_html(positions = indices_results) %>%
kableExtra::pack_rows('Demographics', 1, 20, indent = FALSE, label_row_css = "text-align: left; font-style: italic") %>%
kableExtra::pack_rows('COVID-19 Symptoms', 20, 42, indent = FALSE, label_row_css = "text-align: left; font-style: italic") %>%
kableExtra::pack_rows('Comorbidities', 43, 52, indent = FALSE, label_row_css = "text-align: left; font-style: italic") %>%
as.character() %>%
stringr::str_replace_all(c("Comorbidities"),
            replacement = function(string) paste0(string, '<hr>')) %>%
stringr::str_replace_all(c("Demographics"),
            replacement = function(string) paste0(string, '<hr>')) %>%
stringr::str_replace_all(c("COVID-19 Symptoms"),
            replacement = function(string) paste0(string, '<hr>')) 
```
...
coos_df <- orchid_data %>%
  ...
  tidyr::pivot_longer(cols = starts_with("d_covid"),
               names_to = "date",
               values_to = "COOS") %>%
  mutate(COOS =  forcats::fct_rev(COOS)) %>%
  mutate(COOS = dplyr::recode(COOS, !!!covid_ooscale_renaming))


plot_count_COOS <- coos_df %>%
  dplyr::filter(date %in% c("d_covid15", "d_covid29")) %>%
  mutate(rand_trt = forcats::fct_recode(relevel(rand_trt, 'Hydroxychloroquine'),
                               "Hydroxychloroquine\n(n=242)" = "Hydroxychloroquine",
                               "Placebo\n(n=237)" = "Placebo"),
         date = forcats::fct_recode(date,
  ...
  orchid_gg_theme(plot.title = ggtext::element_markdown()) +
...
line_data <- plot_data[[1]] %>%
    mutate(x_line = dplyr::if_else(x == 1, xmax, xmin), 
           date = as.factor(dplyr::if_else(PANEL == 1, 
                           "14 d After randomization\n(primary outcome)",
                           "28 d After randomization\n(secondary outcome)"
                            ))
          )
...
get_quartiles <- function(outcome, group, data) {
  ...
  df_results <- tidyr::pivot_longer(df_quartiles,
               cols = c("25%", "50%", "75%"), 
               names_to = 'attribute', 
              values_to = 'value') %>%
...
ci_quantreg <- function(outcome, group, difference, df) {
  ...
  qr_b <- quantreg::boot.rq(cbind(1, df[[group]][non_nan]), numeric_var[non_nan], tau = 0.5, R = 10000)
...
get_table_count <- function(data, binary_outcome, safety_outcomes) {
 ...
 table_count <- table(data[non_na, c('rand_trt', binary_outcome)]) %>%
   as.data.frame() %>%
   tidyr::pivot_wider(names_from = all_of(binary_outcome), values_from = 'Freq') %>%
   mutate(freq = round(Yes/(Yes + No), 3) * 100) %>%
   tidyr::pivot_longer(cols = c('No', 'Yes', 'freq'), 
               names_to = 'attribute') %>%
   mutate(entity = 'count') %>%
   dplyr::rename(group = rand_trt)
   ...
   dplyr::inner_join(table_count, df_string, by = "group")         
}
```
...
binary_outcomes_analysis <- function(binary_outcomes,
    ...
    for (binary_outcome in binary_outcomes) {
     ...
     table_count <- get_table_count(data, binary_outcome, safety_outcomes)
      ...
      df_differences <- DescTools::BinomDiffCI(
          x1 = as.numeric(
...
data_manage_table <- function(outcome_comparison) {
    ...
    dplyr::filter(entity %in% c("unadjusted", "adjusted")) %>%
    unique() %>%
    tidyr::pivot_wider(names_from = "entity", values_from = "string")
    outcome_counts <- outcome_comparison[c("string", "outcome", "group", "entity")] %>%
    dplyr::filter(entity %in% c("count")) %>%
    unique() %>%
    tidyr::pivot_wider(names_from = "group", values_from = "string")
    table_results_outcomes <- dplyr::inner_join(outcome_models, outcome_counts, by = "outcome") %>%
      as.data.frame()
...
table_results <- table_results_outcomes %>%
  kableExtra::kable(caption="Effect of Hydroxychloroquine on COVID Outcomes Scale") %>%
  kableExtra::kable_minimal() %>%
  kableExtra::pack_rows('Primary Outcome', 1, 1, indent = FALSE, label_row_css = "text-align: left; font-style: italic") %>%
  kableExtra::pack_rows('Secondary Outcomes', 2, 2, indent = FALSE, label_row_css = "text-align: left; font-style: italic") %>%
  kableExtra::pack_rows('COVID Outcomes Scale score, median (IQR)', 2, 4, indent = TRUE, label_row_css = "text-align: left") %>%
  kableExtra::pack_rows('Support-free days through day 28, median (IQR)', 6, 10, indent = TRUE, label_row_css = "text-align: left") %>%
  kableExtra::pack_rows('All-cause, all-location death, No. (%)', 11, 12, indent = TRUE, label_row_css = "text-align: left") %>%
  kableExtra::pack_rows('Systematically collected safety events, No. (%)', 14, 19, indent = FALSE, label_row_css = "text-align: left; font-style: italic") %>%
  as.character() %>%
stringr::str_replace_all(c("Primary Outcome"),
            replacement = function(string) paste0(string, '<hr>')) %>%
stringr::str_replace_all(c("Secondary Outcomes"),
            replacement = function(string) paste0(string, '<hr>')) %>%
stringr::str_replace_all(c("Systematically collected safety events, No\\. \\(\\%\\)"),
            replacement = function(string) paste0(string, '<hr>')) %>%
stringr::str_replace("Placebo \\(n = 237\\)",
            replacement = "Placebo <br>(n = 237)") %>%
stringr::str_replace("Hydroxychloroquine \\(n = 242\\)",
            replacement = "Hydroxychloroquine <br>(n = 242)") %>%
stringr::str_replace("COVID Outcomes Scale score, median \\(IQR\\) at Day 14",
...
coos_df <- orchid_data %>%
  ...
  tidyr::pivot_longer(cols = c(starts_with("d_covid"), covid_ooscale_1),
               names_to = "date",
               values_to = "COOS") %>%
...
table_count_azithro <- table_outcomes_count %>%
  ...
  tidyr::pivot_wider(id_cols = c(COOS),
...
labs_long <- orchid_data[c("Patient ID", "rand_trt", "prmed_azithro", orchid_labs_names)] %>%
    tidyr::pivot_longer(cols = all_of(orchid_labs_names), 
                 names_sep = "_(?=[0-9]$)", 
                 names_to = c("labs", "day"),
                 values_to = "values") %>%
