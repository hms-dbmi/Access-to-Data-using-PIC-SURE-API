...
*This notebook has been tested to work with R version 4.1.1*. 
...
other attached packages:
 [1] hpds_0.1.1        IRdisplay_1.0     coin_1.4-1        cmprsk_2.2-10    
 [5] DescTools_0.99.41 usethis_2.0.1     kableExtra_1.2.1  survival_3.3-1   
 [9] survminer_0.4.9   ggpubr_0.4.0      MASS_7.3-53.1     quantreg_5.85    
[13] SparseM_1.81      ggtext_0.1.1      urltools_1.7.3    arsenal_3.6.2    
[17] tidyr_1.1.3       ggplot2_3.3.5     forcats_0.5.1     stringr_1.4.0    
[21] dplyr_1.0.9       versions_0.3     

loaded via a namespace (and not attached):
 [1] TH.data_1.0-10     colorspace_2.0-2   ggsignif_0.6.3     ellipsis_0.3.2    
 [5] class_7.3-19       modeltools_0.2-23  rio_0.5.27         base64enc_0.1-3   
 [9] fs_1.5.0           gld_2.6.2          gridtext_0.1.4     rstudioapi_0.13   
[13] proxy_0.4-26       MatrixModels_0.5-0 hash_2.2.6.2       fansi_0.5.0       
[17] mvtnorm_1.1-3      xml2_1.3.2         codetools_0.2-18   splines_4.1.1     
[21] rootSolve_1.8.2.1  libcoin_1.0-8      knitr_1.34         IRkernel_1.2      
[25] jsonlite_1.8.0     broom_0.7.9        km.ci_0.5-6        compiler_4.1.1    
[29] httr_1.4.3         backports_1.2.1    assertthat_0.2.1   Matrix_1.3-4      
[33] fastmap_1.1.0      cli_3.3.0          htmltools_0.5.2    tools_4.1.1       
[37] gtable_0.3.0       glue_1.6.2         lmom_2.8           Rcpp_1.0.9        
[41] carData_3.0-5      cellranger_1.1.0   vctrs_0.4.1        conquer_1.0.2     
[45] xfun_0.26          openxlsx_4.2.4     rvest_1.0.1        lifecycle_1.0.1   
[49] rstatix_0.7.0      zoo_1.8-9          scales_1.1.1       hms_1.1.0         
[53] sandwich_3.0-1     parallel_4.1.1     expm_0.999-6       curl_4.3.2        
[57] Exact_2.1          gridExtra_2.3      KMsurv_0.1-5       triebeard_0.3.0   
[61] stringi_1.7.8      e1071_1.7-8        boot_1.3-28        zip_2.2.0         
[65] repr_1.1.3         rlang_1.0.2        pkgconfig_2.0.3    matrixStats_0.58.0
[69] evaluate_0.14      lattice_0.20-44    purrr_0.3.4        tidyselect_1.1.1  
[73] magrittr_2.0.3     R6_2.5.1           generics_0.1.0     multcomp_1.4-17   
[77] pbdZMQ_0.3-5       DBI_1.1.1          pillar_1.6.2       haven_2.4.3       
[81] foreign_0.8-82     withr_2.4.2        abind_1.4-5        tibble_3.1.4      
[85] crayon_1.4.1       car_3.0-11         survMisc_0.5.6     uuid_0.1-4        
[89] utf8_1.2.2         rmarkdown_2.10     grid_4.1.1         readxl_1.3.1      
[93] data.table_1.14.0  digest_0.6.27      webshot_0.5.2      xtable_1.8-4      
...
```{r}
### SEVEN BRIDGES USERS ONLY: RESTART YOUR KERNEL BEFORE PROCEEDING
```
...
required_libraries <- c(
    ...
    "urltools", 
    "ggtext", # imports ggplot2, grid, gridtext, rland, scales
...
required <- data.frame(Package = required_libraries, Version = required_versions)

installed <- as.data.frame(installed.packages()[,c('Package', 'Version')])

tmp <- merge(required, installed, by = 'Package')
a <- tmp[tmp$Version.x < tmp$Version.y,]
tmp <- merge(required, installed, by = 'Package', all.x = TRUE)
b <- tmp[is.na(tmp$Version.y),]
to_update <- rbind(a,b)


install.versions(to_update$Package, to_update$Version.x)

for (x in c(1:nrow(required))) {
    library(required[x, 'Package'], character.only = TRUE)
}
```
...
dictionary_results <- bdc::find.in.dictionary(dictionary, 'ORCHID')
dictionary_df <- bdc::extract.dataframe(dictionary_results)
list_variables <- dictionary_df$HPDS_PATH

query <- bdc::new.query(authPicSure) # Start a new query
invisible(lapply(list_variables, bdc::query.anyof.add, query = query))
raw_df <- bdc::query.run(query) %>% as_tibble()
```
