---
title: An R Markdown document converted from "/home/ec2-user/SageMaker/Access-to-Data-using-PIC-SURE-API/NHLBI_BioData_Catalyst/R/4_Genomic_Queries.ipynb"
...
It is connected to the specific resource ID we supplied and enables us to query and retrieve data from this database.

## Building the query with the PIC-SURE API

We are going to create a new query request from the PIC-SURE resource that was specified above.  We will limit the query to a single study, a single phenotype (gender and age range) and two genomic filters.
...
Once our query object is finally built, we use the `getResultsDataFrame` function to retrieve the data corresponding to our query.  But first, we want to also know whether the participants that match the previously defined criteria have asthma or not. To select variables to return in the query we can use the data dictionary to search for `Case or Control status for asthma` and add it to our query's selection list.
