---
title: An R Markdown document converted from "/home/ec2-user/SageMaker/Access-to-Data-using-PIC-SURE-API/NHLBI_BioData_Catalyst/R/1_PICSURE_API_101.ipynb"
...
### More about PIC-SURE
The API is available in two different programming languages, python and R, enabling investigators to query the databases the same way using either language. The R/python PIC-SURE API is a small part of the entire PIC-SURE platform. 
...
You should now see a data_dictionary.csv in the RStudio file explorer.

### Parsing variable names

We can use a simple function, `get_multiIndex_variablesDict`, defined in `Rstudio_lib/utils.R` to add a little more information to the variable dictionary and to simplify working with variables names.
...
* bdc::query.select.add() 
    Arguments/input: variable names (string) or list of strings
    Output: all variables included in the list (no record subsetting)
* bdc::query.require.add()
    Arguments/input: variable names (string) or list of strings
    Output: all variables; only records that do not contain null values for input variables |
* bdc::query.anyof.add()
    Arguments/input: variable names (string) or list of strings
    Output: all variables; only records that contain at least one non-null value for input variables
* bdc::query.filter.add()
    Arguments/input: variable name and additional filtering values
    Outpu: input variable; only records that match filter criteria
