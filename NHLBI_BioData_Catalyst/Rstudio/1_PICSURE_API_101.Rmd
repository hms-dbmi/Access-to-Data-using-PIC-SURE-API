...
```{r}
# NOTE: remove limit when not developer
searchResults <- picsure::bdc.searchPicsure(session, limit = 1000) # retrieve all the variables you have access to
```
...
Now, let's find all of the variables associated with that study. We can search for these using the `searchPicsure` function and searching the phs accession number. We are interested in the values associated with the categorical variables in this study, so we will make sure to include the argument `includeValues = TRUE`.
...
# Note: not working, in bdc package
...
```{r}
#first_var <- picsure::bdc.get.paths(my_variables)[1]
...
categorical_var <- categorical_vars[v, 'name']
...
filter_category <- list(trimws(filter_category))
```
...
```{r}
query_categorical_example <- picsure::addClause(query = query_categorical_example, 
                                                keys = categorical_var, 
                                                type = "FILTER", 
...
```{r}
authQuery_categorical_example2 <- picsure::bdc.newQuery(session) # Initialize a new query
authQuery_categorical_example2 <- picsure::addClause(query = authQuery_categorical_example2, 
                                                     keys = categorical_var, 
                                                     type = 'REQUIRE') # use type = REQUIRE to retrieve all data from participants with a value for this variable
full_results_categorical <-  picsure::runQuery(authQuery_categorical_example2)
...
authQuery_continuous_example2 <- picsure::addClause(query = authQuery_continuous_example2, 
                                                   keys = continuous_var, 
                                                   type = 'REQUIRE') # use type = REQUIRE to retreive all data from pariticipants with a value for this variable
...
Tthe output shown above lists all of the phs accession numbers and consent codes that you are authorized to access. 
