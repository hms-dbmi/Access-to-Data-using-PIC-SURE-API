...
devtools::install_github("hms-dbmi/pic-sure-r-adapter-hpds", ref="ALS-9658", force=T, quiet=FALSE)
library(dplyr)
library(picsure)
```
...
Now that you have set up your connection to the PIC-SURE API, let's determine which study or studies you are authorized to access. The `getStudies` function can be used to return a list of the study accession numbers you are authorized to access.

```{r}
searchResults <- picsure::bdc.getStudies(session) # retrieve all the studies you have access to
print(searchResults)
```
